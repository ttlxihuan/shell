...
disk_format(){
    # 获取当前存在的分区
    local PARTS_PATH=`lsblk -nlp $2|awk '$6 == "part" && !$7{print $1}'` END_SECTOR=$SECTOR_NUM
...
EOF
    # sleep 2
    # 格式化完后再获取唯一多出来的分区
    PARTS_PATH="$PARTS_PATH\n"`lsblk -nlp $2|awk '$6 == "part" && !$7{print $1}'`
...
part_mount(){
    ...
    PART_INFO=(`lsblk -nfl $1|awk '{print $1,$2,$4}'|tail -1`)
    if ((${#PART_INFO[@]} < 1));then
        echo "无法获取分区信息：$1";
...
if [ "$ARGV_part_size" != '0' ];then
    if [ -z "$ARGV_part_size" ] || ! [[ "$ARGV_part_size" =~ ^[1-9][0-9]*[BKMGT]?$ ]];then
        ...
        PART_SIZE=$[ $SIZE_UNIT * ${ARGV_part_size:0:-1} ]
...
for ((INDEX=0;INDEX<${#DISKS_ARRAY[@]};INDEX++));do
    ...
    if [ ${ARRAY_DATA[5]} = 'disk' ];then
        if ask_select "硬盘 ${ARRAY_DATA[0]} 未用空间：$UNUSERD_SIZE 是否格式化再挂载？" && disk_format PART_NAME ${ARRAY_DATA[0]};then
            PART_INFO=(`lsblk -nlp $PART_NAME|awk '$6 == "part" && $7 == "" {print $1,$4}'|tail -1`)
